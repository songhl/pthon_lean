...
class zabbixtools:
    def __init__(self):
        self.url = "http://172.16.183.194/api_jsonrpc.php"
        #self.url = "http://192.168.134.180/api_jsonrpc.php"
...
def disenable_item(hostip):
    try:
        conn = MySQLdb.connect('172.16.183.55',port = 3358,user =  'zabbix',passwd =  'zabbixmonitor')
        cursor = conn.cursor()
        sql = '''UPDATE zabbix.hosts a,zabbix.interface b,zabbix.items c SET c.status=1 WHERE a.hostid=b.hostid AND a.hostid=c.hostid AND b.ip='%s' AND c.name IN ('{#FSNAME}.Master_Host','{#FSNAME}.Seconds_Behind_Master','{#FSNAME}.Slave_IO_Running','{#FSNAME}.Slave_SQL_Running');''' % hostip 
        cursor.execute(sql)
        conn.commit()
        cursor.close()
        conn.close()
...
def to_log(text):
    logfile = "/export/data/zabbix/alertscripts/create_host.log"
...
def main(hostip,hostname,group_name,proxyname,type):
    #hostip = sys.argv[1]
    #hostname = sys.argv[2]
    #templates = 'Mutil_Port_Mysql'
    #group_name = 'Ecc_CDB'
    #proxyname = "YF_PROXY"
    zabbix = zabbixtools()
    if "-M-" in hostname:
        if type == 0:
            templates = 'Mysql_Monitor'
        else:
            templates = 'Docker'
        addres = zabbix.add_host(hostname,hostip,group_name,proxyname,templates)
    else:
        if type == 0:
            templates = 'Mysql_Monitor,Mysql_replmonitor'
        else:
            templates = 'Docker,Mysql_replmonitor'
        addres = zabbix.add_host(hostname,hostip,group_name,proxyname,templates)
    return addres
